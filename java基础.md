...
- 指数形式：2e3f   2e3d   以f或d结尾

字符常量

​	‘ a’    表示一个字符，用英文半角格式的单引号

字符串常量

​	“ abcdefg”.  表示一连串的字符，用英文半角格式的双引号
...
向上转型不需要显式的声明，不能通过父类变量去调用子类特有的方法。因为子类已经自动向上转型为了父类对象，就无法调用子类所特有的方法了
